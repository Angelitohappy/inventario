...
class edit_producto():
    ...
    def insert_data(self):
        ...
        try:
            ...
            idProducto = db.read_data_idProducto(cod)
            db.update_data_producto(marca,tipo_neumatico,anchura,perfil,radio,indice_carga,indice_velocidad,str(idProducto[0]))
            db.insert_data_registro_edit(str(idUsuario[0]))
            db.close_connection()
            messagebox.showinfo(title="Aviso!", message="Se ha modificado exitosamente el producto!")
            from interfaces.VENTANA_PRODUCTOS import verProducto
            self.ventana.destroy()
            verProducto(username)
        except Error as e:
    ...
    def __init__(self,username):
        ...
        # Titulo de la pagina
        label1 = tk.Label(frame_superior, text = "EDICION DE PRODUCTO")
        ...
        datos = self.buscar_codProducto()
        self.cod=ttk.Combobox(self.ventana,width=20, height=5 ,values=['prueba'], state="readonly", font = ("Arial", 12))
        self.cod["values"] = [fila[0] for fila in datos]
        self.cod.current(0)
        self.cod.place(x = 75, y = 510)
